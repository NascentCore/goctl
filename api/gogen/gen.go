...
var (
	...
	// VarStringService describes the name of the service.
	VarStringService string
)
...
func DoGenProject(apiFile, dir, style string) error {
	...
	rootPkg, err := golang.GetParentPackage(path.Join(dir, "internal", VarStringService))
	if err != nil {
		return err
	}

	fmt.Printf("rootPkg=%s\n", rootPkg)

	cmdDir := path.Join(dir, "cmd", VarStringService)
	logx.Must(genEtc(cmdDir, cfg, api))
	logx.Must(genMain(cmdDir, rootPkg, cfg, api))

	internalDir := path.Join(dir, "internal", VarStringService)
	logx.Must(genConfig(internalDir, cfg, api))
	logx.Must(genServiceContext(internalDir, rootPkg, cfg, api))
	logx.Must(genTypes(internalDir, cfg, api))
	logx.Must(genRoutes(internalDir, rootPkg, cfg, api))
	logx.Must(genHandlers(internalDir, rootPkg, cfg, api))
	logx.Must(genLogic(internalDir, rootPkg, cfg, api))
	logx.Must(genMiddleware(internalDir, cfg, api))
